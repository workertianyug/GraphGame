...
class HeurAtt(object):
    ...
    def act(self, attState, attNode, eps=0):
        ...
        # if immediate future rewards aren't as good, choose to stay
        if rewards[idx] <= current_reward and current_reward > 0:
            if self.lastAction != None:
                move = self.lastAction[1]
                new_move = (move, move)
                self.lastAction = new_move
                return self.lastAction
