...
# now add in some self stuff

edgeHolder = output_ops_tr[0].edges

edgeHolderT = tf.transpose(edgeHolder) # [1,?]

myHolder = tf.placeholder(tf.float64,shape=[1,5])

exten = tf.concat([edgeHolderT,myHolder],axis=1)

# extenF = tf.reshape(exten,[-1])

extenR = tf.slice(exten,[0,0],[1,15])

lout = tf.layers.dense(extenR,1)

# todo first set up the actor and critic networks

# critic network takes in state and action of all agent, output a single value
# actor network takes in state and output action
